// ./proto/schedule/v1/schedule.proto

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.9
// 	protoc        v6.32.0
// source: proto/schedule/v1/schedule.proto

package schedule

import (
	v11 "github.com/adrien19/chronoqueue/api/common/v1"
	v1 "github.com/adrien19/chronoqueue/api/message/v1"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	durationpb "google.golang.org/protobuf/types/known/durationpb"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type Schedule_Metadata_State int32

const (
	Schedule_Metadata_SCHEDULED Schedule_Metadata_State = 0
	Schedule_Metadata_CANCELED  Schedule_Metadata_State = 1
	Schedule_Metadata_ERRORED   Schedule_Metadata_State = 2
	Schedule_Metadata_PAUSED    Schedule_Metadata_State = 3
)

// Enum value maps for Schedule_Metadata_State.
var (
	Schedule_Metadata_State_name = map[int32]string{
		0: "SCHEDULED",
		1: "CANCELED",
		2: "ERRORED",
		3: "PAUSED",
	}
	Schedule_Metadata_State_value = map[string]int32{
		"SCHEDULED": 0,
		"CANCELED":  1,
		"ERRORED":   2,
		"PAUSED":    3,
	}
)

func (x Schedule_Metadata_State) Enum() *Schedule_Metadata_State {
	p := new(Schedule_Metadata_State)
	*p = x
	return p
}

func (x Schedule_Metadata_State) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (Schedule_Metadata_State) Descriptor() protoreflect.EnumDescriptor {
	return file_proto_schedule_v1_schedule_proto_enumTypes[0].Descriptor()
}

func (Schedule_Metadata_State) Type() protoreflect.EnumType {
	return &file_proto_schedule_v1_schedule_proto_enumTypes[0]
}

func (x Schedule_Metadata_State) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use Schedule_Metadata_State.Descriptor instead.
func (Schedule_Metadata_State) EnumDescriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{0, 0, 0}
}

type CalendarSchedule_ScheduleType int32

const (
	CalendarSchedule_MONTHLY       CalendarSchedule_ScheduleType = 0
	CalendarSchedule_WEEKLY        CalendarSchedule_ScheduleType = 1
	CalendarSchedule_DAILY         CalendarSchedule_ScheduleType = 2
	CalendarSchedule_YEARLY        CalendarSchedule_ScheduleType = 3
	CalendarSchedule_BUSINESS_DAYS CalendarSchedule_ScheduleType = 4
	CalendarSchedule_CUSTOM        CalendarSchedule_ScheduleType = 5
)

// Enum value maps for CalendarSchedule_ScheduleType.
var (
	CalendarSchedule_ScheduleType_name = map[int32]string{
		0: "MONTHLY",
		1: "WEEKLY",
		2: "DAILY",
		3: "YEARLY",
		4: "BUSINESS_DAYS",
		5: "CUSTOM",
	}
	CalendarSchedule_ScheduleType_value = map[string]int32{
		"MONTHLY":       0,
		"WEEKLY":        1,
		"DAILY":         2,
		"YEARLY":        3,
		"BUSINESS_DAYS": 4,
		"CUSTOM":        5,
	}
)

func (x CalendarSchedule_ScheduleType) Enum() *CalendarSchedule_ScheduleType {
	p := new(CalendarSchedule_ScheduleType)
	*p = x
	return p
}

func (x CalendarSchedule_ScheduleType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (CalendarSchedule_ScheduleType) Descriptor() protoreflect.EnumDescriptor {
	return file_proto_schedule_v1_schedule_proto_enumTypes[1].Descriptor()
}

func (CalendarSchedule_ScheduleType) Type() protoreflect.EnumType {
	return &file_proto_schedule_v1_schedule_proto_enumTypes[1]
}

func (x CalendarSchedule_ScheduleType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use CalendarSchedule_ScheduleType.Descriptor instead.
func (CalendarSchedule_ScheduleType) EnumDescriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{2, 0}
}

type MonthlyRule_DayType int32

const (
	MonthlyRule_DAY_OF_MONTH     MonthlyRule_DayType = 0 // e.g., 15th of every month
	MonthlyRule_WEEKDAY_OF_MONTH MonthlyRule_DayType = 1 // e.g., first Monday of month
	MonthlyRule_LAST_WEEKDAY     MonthlyRule_DayType = 2 // e.g., last Friday of month
	MonthlyRule_LAST_DAY         MonthlyRule_DayType = 3 // last day of month
)

// Enum value maps for MonthlyRule_DayType.
var (
	MonthlyRule_DayType_name = map[int32]string{
		0: "DAY_OF_MONTH",
		1: "WEEKDAY_OF_MONTH",
		2: "LAST_WEEKDAY",
		3: "LAST_DAY",
	}
	MonthlyRule_DayType_value = map[string]int32{
		"DAY_OF_MONTH":     0,
		"WEEKDAY_OF_MONTH": 1,
		"LAST_WEEKDAY":     2,
		"LAST_DAY":         3,
	}
)

func (x MonthlyRule_DayType) Enum() *MonthlyRule_DayType {
	p := new(MonthlyRule_DayType)
	*p = x
	return p
}

func (x MonthlyRule_DayType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (MonthlyRule_DayType) Descriptor() protoreflect.EnumDescriptor {
	return file_proto_schedule_v1_schedule_proto_enumTypes[2].Descriptor()
}

func (MonthlyRule_DayType) Type() protoreflect.EnumType {
	return &file_proto_schedule_v1_schedule_proto_enumTypes[2]
}

func (x MonthlyRule_DayType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use MonthlyRule_DayType.Descriptor instead.
func (MonthlyRule_DayType) EnumDescriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{4, 0}
}

type CalendarException_ExceptionType int32

const (
	CalendarException_SKIP       CalendarException_ExceptionType = 0 // skip execution on this date
	CalendarException_RESCHEDULE CalendarException_ExceptionType = 1 // move execution to different date
	CalendarException_EXTRA      CalendarException_ExceptionType = 2 // add extra execution on this date
)

// Enum value maps for CalendarException_ExceptionType.
var (
	CalendarException_ExceptionType_name = map[int32]string{
		0: "SKIP",
		1: "RESCHEDULE",
		2: "EXTRA",
	}
	CalendarException_ExceptionType_value = map[string]int32{
		"SKIP":       0,
		"RESCHEDULE": 1,
		"EXTRA":      2,
	}
)

func (x CalendarException_ExceptionType) Enum() *CalendarException_ExceptionType {
	p := new(CalendarException_ExceptionType)
	*p = x
	return p
}

func (x CalendarException_ExceptionType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (CalendarException_ExceptionType) Descriptor() protoreflect.EnumDescriptor {
	return file_proto_schedule_v1_schedule_proto_enumTypes[3].Descriptor()
}

func (CalendarException_ExceptionType) Type() protoreflect.EnumType {
	return &file_proto_schedule_v1_schedule_proto_enumTypes[3]
}

func (x CalendarException_ExceptionType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use CalendarException_ExceptionType.Descriptor instead.
func (CalendarException_ExceptionType) EnumDescriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{17, 0}
}

type Schedule struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ScheduleId    string                 `protobuf:"bytes,1,opt,name=schedule_id,json=scheduleId,proto3" json:"schedule_id,omitempty"`
	Metadata      *Schedule_Metadata     `protobuf:"bytes,2,opt,name=metadata,proto3" json:"metadata,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Schedule) Reset() {
	*x = Schedule{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Schedule) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Schedule) ProtoMessage() {}

func (x *Schedule) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Schedule.ProtoReflect.Descriptor instead.
func (*Schedule) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{0}
}

func (x *Schedule) GetScheduleId() string {
	if x != nil {
		return x.ScheduleId
	}
	return ""
}

func (x *Schedule) GetMetadata() *Schedule_Metadata {
	if x != nil {
		return x.Metadata
	}
	return nil
}

type ScheduleHistory struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Messages      []*v1.Message          `protobuf:"bytes,1,rep,name=messages,proto3" json:"messages,omitempty"`
	ScheduleId    string                 `protobuf:"bytes,2,opt,name=schedule_id,json=scheduleId,proto3" json:"schedule_id,omitempty"`
	NextRun       *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=next_run,json=nextRun,proto3" json:"next_run,omitempty"`
	LastRun       *timestamppb.Timestamp `protobuf:"bytes,4,opt,name=last_run,json=lastRun,proto3" json:"last_run,omitempty"`
	CreatedAt     *timestamppb.Timestamp `protobuf:"bytes,5,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	UpdatedAt     *timestamppb.Timestamp `protobuf:"bytes,6,opt,name=updated_at,json=updatedAt,proto3" json:"updated_at,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ScheduleHistory) Reset() {
	*x = ScheduleHistory{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ScheduleHistory) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ScheduleHistory) ProtoMessage() {}

func (x *ScheduleHistory) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ScheduleHistory.ProtoReflect.Descriptor instead.
func (*ScheduleHistory) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{1}
}

func (x *ScheduleHistory) GetMessages() []*v1.Message {
	if x != nil {
		return x.Messages
	}
	return nil
}

func (x *ScheduleHistory) GetScheduleId() string {
	if x != nil {
		return x.ScheduleId
	}
	return ""
}

func (x *ScheduleHistory) GetNextRun() *timestamppb.Timestamp {
	if x != nil {
		return x.NextRun
	}
	return nil
}

func (x *ScheduleHistory) GetLastRun() *timestamppb.Timestamp {
	if x != nil {
		return x.LastRun
	}
	return nil
}

func (x *ScheduleHistory) GetCreatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.CreatedAt
	}
	return nil
}

func (x *ScheduleHistory) GetUpdatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdatedAt
	}
	return nil
}

// Calendar-based scheduling definitions
type CalendarSchedule struct {
	state            protoimpl.MessageState        `protogen:"open.v1"`
	Type             CalendarSchedule_ScheduleType `protobuf:"varint,1,opt,name=type,proto3,enum=chronoqueue.api.schedule.v1.CalendarSchedule_ScheduleType" json:"type,omitempty"`
	Rules            []*CalendarRule               `protobuf:"bytes,2,rep,name=rules,proto3" json:"rules,omitempty"`
	Timezone         string                        `protobuf:"bytes,3,opt,name=timezone,proto3" json:"timezone,omitempty"`
	BusinessCalendar *BusinessCalendar             `protobuf:"bytes,4,opt,name=business_calendar,json=businessCalendar,proto3" json:"business_calendar,omitempty"`
	Exceptions       []*CalendarException          `protobuf:"bytes,5,rep,name=exceptions,proto3" json:"exceptions,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *CalendarSchedule) Reset() {
	*x = CalendarSchedule{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CalendarSchedule) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CalendarSchedule) ProtoMessage() {}

func (x *CalendarSchedule) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CalendarSchedule.ProtoReflect.Descriptor instead.
func (*CalendarSchedule) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{2}
}

func (x *CalendarSchedule) GetType() CalendarSchedule_ScheduleType {
	if x != nil {
		return x.Type
	}
	return CalendarSchedule_MONTHLY
}

func (x *CalendarSchedule) GetRules() []*CalendarRule {
	if x != nil {
		return x.Rules
	}
	return nil
}

func (x *CalendarSchedule) GetTimezone() string {
	if x != nil {
		return x.Timezone
	}
	return ""
}

func (x *CalendarSchedule) GetBusinessCalendar() *BusinessCalendar {
	if x != nil {
		return x.BusinessCalendar
	}
	return nil
}

func (x *CalendarSchedule) GetExceptions() []*CalendarException {
	if x != nil {
		return x.Exceptions
	}
	return nil
}

type CalendarRule struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Rule:
	//
	//	*CalendarRule_Monthly
	//	*CalendarRule_Weekly
	//	*CalendarRule_Daily
	//	*CalendarRule_Yearly
	//	*CalendarRule_BusinessDays
	//	*CalendarRule_Custom
	Rule isCalendarRule_Rule `protobuf_oneof:"rule"`
	// Time of day for execution
	ExecutionTimes []*TimeOfDay `protobuf:"bytes,7,rep,name=execution_times,json=executionTimes,proto3" json:"execution_times,omitempty"`
	// Optional date range for rule validity
	ValidFrom     *timestamppb.Timestamp `protobuf:"bytes,8,opt,name=valid_from,json=validFrom,proto3" json:"valid_from,omitempty"`
	ValidUntil    *timestamppb.Timestamp `protobuf:"bytes,9,opt,name=valid_until,json=validUntil,proto3" json:"valid_until,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CalendarRule) Reset() {
	*x = CalendarRule{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CalendarRule) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CalendarRule) ProtoMessage() {}

func (x *CalendarRule) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CalendarRule.ProtoReflect.Descriptor instead.
func (*CalendarRule) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{3}
}

func (x *CalendarRule) GetRule() isCalendarRule_Rule {
	if x != nil {
		return x.Rule
	}
	return nil
}

func (x *CalendarRule) GetMonthly() *MonthlyRule {
	if x != nil {
		if x, ok := x.Rule.(*CalendarRule_Monthly); ok {
			return x.Monthly
		}
	}
	return nil
}

func (x *CalendarRule) GetWeekly() *WeeklyRule {
	if x != nil {
		if x, ok := x.Rule.(*CalendarRule_Weekly); ok {
			return x.Weekly
		}
	}
	return nil
}

func (x *CalendarRule) GetDaily() *DailyRule {
	if x != nil {
		if x, ok := x.Rule.(*CalendarRule_Daily); ok {
			return x.Daily
		}
	}
	return nil
}

func (x *CalendarRule) GetYearly() *YearlyRule {
	if x != nil {
		if x, ok := x.Rule.(*CalendarRule_Yearly); ok {
			return x.Yearly
		}
	}
	return nil
}

func (x *CalendarRule) GetBusinessDays() *BusinessDaysRule {
	if x != nil {
		if x, ok := x.Rule.(*CalendarRule_BusinessDays); ok {
			return x.BusinessDays
		}
	}
	return nil
}

func (x *CalendarRule) GetCustom() *CustomRule {
	if x != nil {
		if x, ok := x.Rule.(*CalendarRule_Custom); ok {
			return x.Custom
		}
	}
	return nil
}

func (x *CalendarRule) GetExecutionTimes() []*TimeOfDay {
	if x != nil {
		return x.ExecutionTimes
	}
	return nil
}

func (x *CalendarRule) GetValidFrom() *timestamppb.Timestamp {
	if x != nil {
		return x.ValidFrom
	}
	return nil
}

func (x *CalendarRule) GetValidUntil() *timestamppb.Timestamp {
	if x != nil {
		return x.ValidUntil
	}
	return nil
}

type isCalendarRule_Rule interface {
	isCalendarRule_Rule()
}

type CalendarRule_Monthly struct {
	Monthly *MonthlyRule `protobuf:"bytes,1,opt,name=monthly,proto3,oneof"`
}

type CalendarRule_Weekly struct {
	Weekly *WeeklyRule `protobuf:"bytes,2,opt,name=weekly,proto3,oneof"`
}

type CalendarRule_Daily struct {
	Daily *DailyRule `protobuf:"bytes,3,opt,name=daily,proto3,oneof"`
}

type CalendarRule_Yearly struct {
	Yearly *YearlyRule `protobuf:"bytes,4,opt,name=yearly,proto3,oneof"`
}

type CalendarRule_BusinessDays struct {
	BusinessDays *BusinessDaysRule `protobuf:"bytes,5,opt,name=business_days,json=businessDays,proto3,oneof"`
}

type CalendarRule_Custom struct {
	Custom *CustomRule `protobuf:"bytes,6,opt,name=custom,proto3,oneof"`
}

func (*CalendarRule_Monthly) isCalendarRule_Rule() {}

func (*CalendarRule_Weekly) isCalendarRule_Rule() {}

func (*CalendarRule_Daily) isCalendarRule_Rule() {}

func (*CalendarRule_Yearly) isCalendarRule_Rule() {}

func (*CalendarRule_BusinessDays) isCalendarRule_Rule() {}

func (*CalendarRule_Custom) isCalendarRule_Rule() {}

type MonthlyRule struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	DayType       MonthlyRule_DayType    `protobuf:"varint,1,opt,name=day_type,json=dayType,proto3,enum=chronoqueue.api.schedule.v1.MonthlyRule_DayType" json:"day_type,omitempty"`
	DayValue      int32                  `protobuf:"varint,2,opt,name=day_value,json=dayValue,proto3" json:"day_value,omitempty"` // day number (1-31) or weekday (1-7)
	Occurrence    int32                  `protobuf:"varint,3,opt,name=occurrence,proto3" json:"occurrence,omitempty"`             // for WEEKDAY_OF_MONTH: 1=first, 2=second, etc.
	Months        []int32                `protobuf:"varint,4,rep,packed,name=months,proto3" json:"months,omitempty"`              // specific months (1-12), empty = all months
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MonthlyRule) Reset() {
	*x = MonthlyRule{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MonthlyRule) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MonthlyRule) ProtoMessage() {}

func (x *MonthlyRule) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MonthlyRule.ProtoReflect.Descriptor instead.
func (*MonthlyRule) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{4}
}

func (x *MonthlyRule) GetDayType() MonthlyRule_DayType {
	if x != nil {
		return x.DayType
	}
	return MonthlyRule_DAY_OF_MONTH
}

func (x *MonthlyRule) GetDayValue() int32 {
	if x != nil {
		return x.DayValue
	}
	return 0
}

func (x *MonthlyRule) GetOccurrence() int32 {
	if x != nil {
		return x.Occurrence
	}
	return 0
}

func (x *MonthlyRule) GetMonths() []int32 {
	if x != nil {
		return x.Months
	}
	return nil
}

type WeeklyRule struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	DaysOfWeek    []int32                `protobuf:"varint,1,rep,packed,name=days_of_week,json=daysOfWeek,proto3" json:"days_of_week,omitempty"` // 1=Monday, 7=Sunday
	WeekInterval  int32                  `protobuf:"varint,2,opt,name=week_interval,json=weekInterval,proto3" json:"week_interval,omitempty"`    // every N weeks (default: 1)
	StartWeek     *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=start_week,json=startWeek,proto3" json:"start_week,omitempty"`              // reference week for intervals
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WeeklyRule) Reset() {
	*x = WeeklyRule{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WeeklyRule) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WeeklyRule) ProtoMessage() {}

func (x *WeeklyRule) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WeeklyRule.ProtoReflect.Descriptor instead.
func (*WeeklyRule) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{5}
}

func (x *WeeklyRule) GetDaysOfWeek() []int32 {
	if x != nil {
		return x.DaysOfWeek
	}
	return nil
}

func (x *WeeklyRule) GetWeekInterval() int32 {
	if x != nil {
		return x.WeekInterval
	}
	return 0
}

func (x *WeeklyRule) GetStartWeek() *timestamppb.Timestamp {
	if x != nil {
		return x.StartWeek
	}
	return nil
}

type DailyRule struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	DayInterval   int32                  `protobuf:"varint,1,opt,name=day_interval,json=dayInterval,proto3" json:"day_interval,omitempty"`    // every N days (default: 1)
	WeekdaysOnly  bool                   `protobuf:"varint,2,opt,name=weekdays_only,json=weekdaysOnly,proto3" json:"weekdays_only,omitempty"` // exclude weekends
	StartDate     *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=start_date,json=startDate,proto3" json:"start_date,omitempty"`           // reference date for intervals
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DailyRule) Reset() {
	*x = DailyRule{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DailyRule) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DailyRule) ProtoMessage() {}

func (x *DailyRule) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DailyRule.ProtoReflect.Descriptor instead.
func (*DailyRule) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{6}
}

func (x *DailyRule) GetDayInterval() int32 {
	if x != nil {
		return x.DayInterval
	}
	return 0
}

func (x *DailyRule) GetWeekdaysOnly() bool {
	if x != nil {
		return x.WeekdaysOnly
	}
	return false
}

func (x *DailyRule) GetStartDate() *timestamppb.Timestamp {
	if x != nil {
		return x.StartDate
	}
	return nil
}

type YearlyRule struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	Month             int32                  `protobuf:"varint,1,opt,name=month,proto3" json:"month,omitempty"`                                                      // month (1-12)
	Day               int32                  `protobuf:"varint,2,opt,name=day,proto3" json:"day,omitempty"`                                                          // day of month (1-31)
	AdjustForLeapYear bool                   `protobuf:"varint,3,opt,name=adjust_for_leap_year,json=adjustForLeapYear,proto3" json:"adjust_for_leap_year,omitempty"` // handle Feb 29 on non-leap years
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *YearlyRule) Reset() {
	*x = YearlyRule{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *YearlyRule) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*YearlyRule) ProtoMessage() {}

func (x *YearlyRule) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use YearlyRule.ProtoReflect.Descriptor instead.
func (*YearlyRule) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{7}
}

func (x *YearlyRule) GetMonth() int32 {
	if x != nil {
		return x.Month
	}
	return 0
}

func (x *YearlyRule) GetDay() int32 {
	if x != nil {
		return x.Day
	}
	return 0
}

func (x *YearlyRule) GetAdjustForLeapYear() bool {
	if x != nil {
		return x.AdjustForLeapYear
	}
	return false
}

type BusinessDaysRule struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	BusinessCalendarId string                 `protobuf:"bytes,1,opt,name=business_calendar_id,json=businessCalendarId,proto3" json:"business_calendar_id,omitempty"` // reference to business calendar
	DayOffset          int32                  `protobuf:"varint,2,opt,name=day_offset,json=dayOffset,proto3" json:"day_offset,omitempty"`                             // offset from business day (e.g., -1 for day before)
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *BusinessDaysRule) Reset() {
	*x = BusinessDaysRule{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BusinessDaysRule) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BusinessDaysRule) ProtoMessage() {}

func (x *BusinessDaysRule) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BusinessDaysRule.ProtoReflect.Descriptor instead.
func (*BusinessDaysRule) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{8}
}

func (x *BusinessDaysRule) GetBusinessCalendarId() string {
	if x != nil {
		return x.BusinessCalendarId
	}
	return ""
}

func (x *BusinessDaysRule) GetDayOffset() int32 {
	if x != nil {
		return x.DayOffset
	}
	return 0
}

type CustomRule struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Expression    string                 `protobuf:"bytes,1,opt,name=expression,proto3" json:"expression,omitempty"`                                                                           // custom expression for complex rules
	RuleType      string                 `protobuf:"bytes,2,opt,name=rule_type,json=ruleType,proto3" json:"rule_type,omitempty"`                                                               // identifier for custom rule processor
	Parameters    map[string]string      `protobuf:"bytes,3,rep,name=parameters,proto3" json:"parameters,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"` // rule-specific parameters
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CustomRule) Reset() {
	*x = CustomRule{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CustomRule) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CustomRule) ProtoMessage() {}

func (x *CustomRule) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CustomRule.ProtoReflect.Descriptor instead.
func (*CustomRule) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{9}
}

func (x *CustomRule) GetExpression() string {
	if x != nil {
		return x.Expression
	}
	return ""
}

func (x *CustomRule) GetRuleType() string {
	if x != nil {
		return x.RuleType
	}
	return ""
}

func (x *CustomRule) GetParameters() map[string]string {
	if x != nil {
		return x.Parameters
	}
	return nil
}

type TimeOfDay struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Hour          int32                  `protobuf:"varint,1,opt,name=hour,proto3" json:"hour,omitempty"`     // 0-23
	Minute        int32                  `protobuf:"varint,2,opt,name=minute,proto3" json:"minute,omitempty"` // 0-59
	Second        int32                  `protobuf:"varint,3,opt,name=second,proto3" json:"second,omitempty"` // 0-59 (optional, default: 0)
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TimeOfDay) Reset() {
	*x = TimeOfDay{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TimeOfDay) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TimeOfDay) ProtoMessage() {}

func (x *TimeOfDay) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TimeOfDay.ProtoReflect.Descriptor instead.
func (*TimeOfDay) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{10}
}

func (x *TimeOfDay) GetHour() int32 {
	if x != nil {
		return x.Hour
	}
	return 0
}

func (x *TimeOfDay) GetMinute() int32 {
	if x != nil {
		return x.Minute
	}
	return 0
}

func (x *TimeOfDay) GetSecond() int32 {
	if x != nil {
		return x.Second
	}
	return 0
}

type BusinessCalendar struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	CalendarId    string                 `protobuf:"bytes,1,opt,name=calendar_id,json=calendarId,proto3" json:"calendar_id,omitempty"`
	Name          string                 `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Description   string                 `protobuf:"bytes,3,opt,name=description,proto3" json:"description,omitempty"`
	Holidays      []*Holiday             `protobuf:"bytes,4,rep,name=holidays,proto3" json:"holidays,omitempty"`
	WeekendDays   []int32                `protobuf:"varint,5,rep,packed,name=weekend_days,json=weekendDays,proto3" json:"weekend_days,omitempty"` // 1=Monday, 7=Sunday
	Timezone      string                 `protobuf:"bytes,6,opt,name=timezone,proto3" json:"timezone,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BusinessCalendar) Reset() {
	*x = BusinessCalendar{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BusinessCalendar) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BusinessCalendar) ProtoMessage() {}

func (x *BusinessCalendar) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BusinessCalendar.ProtoReflect.Descriptor instead.
func (*BusinessCalendar) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{11}
}

func (x *BusinessCalendar) GetCalendarId() string {
	if x != nil {
		return x.CalendarId
	}
	return ""
}

func (x *BusinessCalendar) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *BusinessCalendar) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *BusinessCalendar) GetHolidays() []*Holiday {
	if x != nil {
		return x.Holidays
	}
	return nil
}

func (x *BusinessCalendar) GetWeekendDays() []int32 {
	if x != nil {
		return x.WeekendDays
	}
	return nil
}

func (x *BusinessCalendar) GetTimezone() string {
	if x != nil {
		return x.Timezone
	}
	return ""
}

type Holiday struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	Name            string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Date            *timestamppb.Timestamp `protobuf:"bytes,2,opt,name=date,proto3" json:"date,omitempty"`
	RecurringYearly bool                   `protobuf:"varint,3,opt,name=recurring_yearly,json=recurringYearly,proto3" json:"recurring_yearly,omitempty"`
	Rule            *HolidayRule           `protobuf:"bytes,4,opt,name=rule,proto3" json:"rule,omitempty"` // for complex holidays like "first Monday in May"
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *Holiday) Reset() {
	*x = Holiday{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Holiday) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Holiday) ProtoMessage() {}

func (x *Holiday) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Holiday.ProtoReflect.Descriptor instead.
func (*Holiday) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{12}
}

func (x *Holiday) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Holiday) GetDate() *timestamppb.Timestamp {
	if x != nil {
		return x.Date
	}
	return nil
}

func (x *Holiday) GetRecurringYearly() bool {
	if x != nil {
		return x.RecurringYearly
	}
	return false
}

func (x *Holiday) GetRule() *HolidayRule {
	if x != nil {
		return x.Rule
	}
	return nil
}

type HolidayRule struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Rule:
	//
	//	*HolidayRule_Fixed
	//	*HolidayRule_Relative
	//	*HolidayRule_EasterOffset
	Rule          isHolidayRule_Rule `protobuf_oneof:"rule"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *HolidayRule) Reset() {
	*x = HolidayRule{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *HolidayRule) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*HolidayRule) ProtoMessage() {}

func (x *HolidayRule) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use HolidayRule.ProtoReflect.Descriptor instead.
func (*HolidayRule) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{13}
}

func (x *HolidayRule) GetRule() isHolidayRule_Rule {
	if x != nil {
		return x.Rule
	}
	return nil
}

func (x *HolidayRule) GetFixed() *FixedDate {
	if x != nil {
		if x, ok := x.Rule.(*HolidayRule_Fixed); ok {
			return x.Fixed
		}
	}
	return nil
}

func (x *HolidayRule) GetRelative() *RelativeDate {
	if x != nil {
		if x, ok := x.Rule.(*HolidayRule_Relative); ok {
			return x.Relative
		}
	}
	return nil
}

func (x *HolidayRule) GetEasterOffset() *EasterOffset {
	if x != nil {
		if x, ok := x.Rule.(*HolidayRule_EasterOffset); ok {
			return x.EasterOffset
		}
	}
	return nil
}

type isHolidayRule_Rule interface {
	isHolidayRule_Rule()
}

type HolidayRule_Fixed struct {
	Fixed *FixedDate `protobuf:"bytes,1,opt,name=fixed,proto3,oneof"`
}

type HolidayRule_Relative struct {
	Relative *RelativeDate `protobuf:"bytes,2,opt,name=relative,proto3,oneof"`
}

type HolidayRule_EasterOffset struct {
	EasterOffset *EasterOffset `protobuf:"bytes,3,opt,name=easter_offset,json=easterOffset,proto3,oneof"`
}

func (*HolidayRule_Fixed) isHolidayRule_Rule() {}

func (*HolidayRule_Relative) isHolidayRule_Rule() {}

func (*HolidayRule_EasterOffset) isHolidayRule_Rule() {}

type FixedDate struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Month         int32                  `protobuf:"varint,1,opt,name=month,proto3" json:"month,omitempty"` // 1-12
	Day           int32                  `protobuf:"varint,2,opt,name=day,proto3" json:"day,omitempty"`     // 1-31
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *FixedDate) Reset() {
	*x = FixedDate{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FixedDate) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FixedDate) ProtoMessage() {}

func (x *FixedDate) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FixedDate.ProtoReflect.Descriptor instead.
func (*FixedDate) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{14}
}

func (x *FixedDate) GetMonth() int32 {
	if x != nil {
		return x.Month
	}
	return 0
}

func (x *FixedDate) GetDay() int32 {
	if x != nil {
		return x.Day
	}
	return 0
}

type RelativeDate struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Month         int32                  `protobuf:"varint,1,opt,name=month,proto3" json:"month,omitempty"`           // 1-12
	Weekday       int32                  `protobuf:"varint,2,opt,name=weekday,proto3" json:"weekday,omitempty"`       // 1=Monday, 7=Sunday
	Occurrence    int32                  `protobuf:"varint,3,opt,name=occurrence,proto3" json:"occurrence,omitempty"` // 1=first, 2=second, -1=last
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RelativeDate) Reset() {
	*x = RelativeDate{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RelativeDate) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RelativeDate) ProtoMessage() {}

func (x *RelativeDate) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RelativeDate.ProtoReflect.Descriptor instead.
func (*RelativeDate) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{15}
}

func (x *RelativeDate) GetMonth() int32 {
	if x != nil {
		return x.Month
	}
	return 0
}

func (x *RelativeDate) GetWeekday() int32 {
	if x != nil {
		return x.Weekday
	}
	return 0
}

func (x *RelativeDate) GetOccurrence() int32 {
	if x != nil {
		return x.Occurrence
	}
	return 0
}

type EasterOffset struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	DaysOffset    int32                  `protobuf:"varint,1,opt,name=days_offset,json=daysOffset,proto3" json:"days_offset,omitempty"` // days from Easter Sunday
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EasterOffset) Reset() {
	*x = EasterOffset{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EasterOffset) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EasterOffset) ProtoMessage() {}

func (x *EasterOffset) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EasterOffset.ProtoReflect.Descriptor instead.
func (*EasterOffset) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{16}
}

func (x *EasterOffset) GetDaysOffset() int32 {
	if x != nil {
		return x.DaysOffset
	}
	return 0
}

type CalendarException struct {
	state         protoimpl.MessageState          `protogen:"open.v1"`
	Date          *timestamppb.Timestamp          `protobuf:"bytes,1,opt,name=date,proto3" json:"date,omitempty"`
	Type          CalendarException_ExceptionType `protobuf:"varint,2,opt,name=type,proto3,enum=chronoqueue.api.schedule.v1.CalendarException_ExceptionType" json:"type,omitempty"`
	RescheduleTo  *timestamppb.Timestamp          `protobuf:"bytes,3,opt,name=reschedule_to,json=rescheduleTo,proto3" json:"reschedule_to,omitempty"` // for RESCHEDULE type
	ExtraTimes    []*TimeOfDay                    `protobuf:"bytes,4,rep,name=extra_times,json=extraTimes,proto3" json:"extra_times,omitempty"`       // for EXTRA type
	Reason        string                          `protobuf:"bytes,5,opt,name=reason,proto3" json:"reason,omitempty"`                                 // optional description
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CalendarException) Reset() {
	*x = CalendarException{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CalendarException) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CalendarException) ProtoMessage() {}

func (x *CalendarException) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CalendarException.ProtoReflect.Descriptor instead.
func (*CalendarException) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{17}
}

func (x *CalendarException) GetDate() *timestamppb.Timestamp {
	if x != nil {
		return x.Date
	}
	return nil
}

func (x *CalendarException) GetType() CalendarException_ExceptionType {
	if x != nil {
		return x.Type
	}
	return CalendarException_SKIP
}

func (x *CalendarException) GetRescheduleTo() *timestamppb.Timestamp {
	if x != nil {
		return x.RescheduleTo
	}
	return nil
}

func (x *CalendarException) GetExtraTimes() []*TimeOfDay {
	if x != nil {
		return x.ExtraTimes
	}
	return nil
}

func (x *CalendarException) GetReason() string {
	if x != nil {
		return x.Reason
	}
	return ""
}

type Schedule_Metadata struct {
	state   protoimpl.MessageState  `protogen:"open.v1"`
	Payload *v11.Payload            `protobuf:"bytes,1,opt,name=payload,proto3" json:"payload,omitempty"`
	State   Schedule_Metadata_State `protobuf:"varint,2,opt,name=state,proto3,enum=chronoqueue.api.schedule.v1.Schedule_Metadata_State" json:"state,omitempty"`
	// Schedule configuration - supports both cron and calendar scheduling
	//
	// Types that are valid to be assigned to ScheduleConfig:
	//
	//	*Schedule_Metadata_CronSchedule
	//	*Schedule_Metadata_CalendarSchedule
	ScheduleConfig isSchedule_Metadata_ScheduleConfig `protobuf_oneof:"schedule_config"`
	QueueName      string                             `protobuf:"bytes,4,opt,name=queue_name,json=queueName,proto3" json:"queue_name,omitempty"`
	MessageIds     []string                           `protobuf:"bytes,5,rep,name=message_ids,json=messageIds,proto3" json:"message_ids,omitempty"`
	NextRun        *timestamppb.Timestamp             `protobuf:"bytes,6,opt,name=next_run,json=nextRun,proto3" json:"next_run,omitempty"`
	LastRun        *timestamppb.Timestamp             `protobuf:"bytes,7,opt,name=last_run,json=lastRun,proto3" json:"last_run,omitempty"`
	CreatedAt      *timestamppb.Timestamp             `protobuf:"bytes,8,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	UpdatedAt      *timestamppb.Timestamp             `protobuf:"bytes,9,opt,name=updated_at,json=updatedAt,proto3" json:"updated_at,omitempty"`
	ExclusivityKey string                             `protobuf:"bytes,10,opt,name=exclusivity_key,json=exclusivityKey,proto3" json:"exclusivity_key,omitempty"`
	StateMessage   string                             `protobuf:"bytes,11,opt,name=state_message,json=stateMessage,proto3" json:"state_message,omitempty"`
	Priority       int64                              `protobuf:"varint,12,opt,name=priority,proto3" json:"priority,omitempty"`
	HasMaxMessages bool                               `protobuf:"varint,13,opt,name=has_max_messages,json=hasMaxMessages,proto3" json:"has_max_messages,omitempty"`
	MaxMessages    int64                              `protobuf:"varint,14,opt,name=max_messages,json=maxMessages,proto3" json:"max_messages,omitempty"`
	LeaseDuration  *durationpb.Duration               `protobuf:"bytes,15,opt,name=lease_duration,json=leaseDuration,proto3" json:"lease_duration,omitempty"`
	// Calendar scheduling metadata
	Timezone      string                   `protobuf:"bytes,17,opt,name=timezone,proto3" json:"timezone,omitempty"`                 // timezone for calendar calculations
	NextRuns      []*timestamppb.Timestamp `protobuf:"bytes,18,rep,name=next_runs,json=nextRuns,proto3" json:"next_runs,omitempty"` // precomputed next execution times
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Schedule_Metadata) Reset() {
	*x = Schedule_Metadata{}
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Schedule_Metadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Schedule_Metadata) ProtoMessage() {}

func (x *Schedule_Metadata) ProtoReflect() protoreflect.Message {
	mi := &file_proto_schedule_v1_schedule_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Schedule_Metadata.ProtoReflect.Descriptor instead.
func (*Schedule_Metadata) Descriptor() ([]byte, []int) {
	return file_proto_schedule_v1_schedule_proto_rawDescGZIP(), []int{0, 0}
}

func (x *Schedule_Metadata) GetPayload() *v11.Payload {
	if x != nil {
		return x.Payload
	}
	return nil
}

func (x *Schedule_Metadata) GetState() Schedule_Metadata_State {
	if x != nil {
		return x.State
	}
	return Schedule_Metadata_SCHEDULED
}

func (x *Schedule_Metadata) GetScheduleConfig() isSchedule_Metadata_ScheduleConfig {
	if x != nil {
		return x.ScheduleConfig
	}
	return nil
}

func (x *Schedule_Metadata) GetCronSchedule() string {
	if x != nil {
		if x, ok := x.ScheduleConfig.(*Schedule_Metadata_CronSchedule); ok {
			return x.CronSchedule
		}
	}
	return ""
}

func (x *Schedule_Metadata) GetCalendarSchedule() *CalendarSchedule {
	if x != nil {
		if x, ok := x.ScheduleConfig.(*Schedule_Metadata_CalendarSchedule); ok {
			return x.CalendarSchedule
		}
	}
	return nil
}

func (x *Schedule_Metadata) GetQueueName() string {
	if x != nil {
		return x.QueueName
	}
	return ""
}

func (x *Schedule_Metadata) GetMessageIds() []string {
	if x != nil {
		return x.MessageIds
	}
	return nil
}

func (x *Schedule_Metadata) GetNextRun() *timestamppb.Timestamp {
	if x != nil {
		return x.NextRun
	}
	return nil
}

func (x *Schedule_Metadata) GetLastRun() *timestamppb.Timestamp {
	if x != nil {
		return x.LastRun
	}
	return nil
}

func (x *Schedule_Metadata) GetCreatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.CreatedAt
	}
	return nil
}

func (x *Schedule_Metadata) GetUpdatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdatedAt
	}
	return nil
}

func (x *Schedule_Metadata) GetExclusivityKey() string {
	if x != nil {
		return x.ExclusivityKey
	}
	return ""
}

func (x *Schedule_Metadata) GetStateMessage() string {
	if x != nil {
		return x.StateMessage
	}
	return ""
}

func (x *Schedule_Metadata) GetPriority() int64 {
	if x != nil {
		return x.Priority
	}
	return 0
}

func (x *Schedule_Metadata) GetHasMaxMessages() bool {
	if x != nil {
		return x.HasMaxMessages
	}
	return false
}

func (x *Schedule_Metadata) GetMaxMessages() int64 {
	if x != nil {
		return x.MaxMessages
	}
	return 0
}

func (x *Schedule_Metadata) GetLeaseDuration() *durationpb.Duration {
	if x != nil {
		return x.LeaseDuration
	}
	return nil
}

func (x *Schedule_Metadata) GetTimezone() string {
	if x != nil {
		return x.Timezone
	}
	return ""
}

func (x *Schedule_Metadata) GetNextRuns() []*timestamppb.Timestamp {
	if x != nil {
		return x.NextRuns
	}
	return nil
}

type isSchedule_Metadata_ScheduleConfig interface {
	isSchedule_Metadata_ScheduleConfig()
}

type Schedule_Metadata_CronSchedule struct {
	CronSchedule string `protobuf:"bytes,3,opt,name=cron_schedule,json=cronSchedule,proto3,oneof"` // legacy cron-based scheduling
}

type Schedule_Metadata_CalendarSchedule struct {
	CalendarSchedule *CalendarSchedule `protobuf:"bytes,16,opt,name=calendar_schedule,json=calendarSchedule,proto3,oneof"` // new calendar-based scheduling
}

func (*Schedule_Metadata_CronSchedule) isSchedule_Metadata_ScheduleConfig() {}

func (*Schedule_Metadata_CalendarSchedule) isSchedule_Metadata_ScheduleConfig() {}

var File_proto_schedule_v1_schedule_proto protoreflect.FileDescriptor

const file_proto_schedule_v1_schedule_proto_rawDesc = "" +
	"\n" +
	" proto/schedule/v1/schedule.proto\x12\x1bchronoqueue.api.schedule.v1\x1a\x1fgoogle/protobuf/timestamp.proto\x1a\x1egoogle/protobuf/duration.proto\x1a\x1cproto/common/v1/common.proto\x1a\x1eproto/message/v1/message.proto\"\xd7\b\n" +
	"\bSchedule\x12\x1f\n" +
	"\vschedule_id\x18\x01 \x01(\tR\n" +
	"scheduleId\x12J\n" +
	"\bmetadata\x18\x02 \x01(\v2..chronoqueue.api.schedule.v1.Schedule.MetadataR\bmetadata\x1a\xdd\a\n" +
	"\bMetadata\x12<\n" +
	"\apayload\x18\x01 \x01(\v2\".chronoqueue.api.common.v1.PayloadR\apayload\x12J\n" +
	"\x05state\x18\x02 \x01(\x0e24.chronoqueue.api.schedule.v1.Schedule.Metadata.StateR\x05state\x12%\n" +
	"\rcron_schedule\x18\x03 \x01(\tH\x00R\fcronSchedule\x12\\\n" +
	"\x11calendar_schedule\x18\x10 \x01(\v2-.chronoqueue.api.schedule.v1.CalendarScheduleH\x00R\x10calendarSchedule\x12\x1d\n" +
	"\n" +
	"queue_name\x18\x04 \x01(\tR\tqueueName\x12\x1f\n" +
	"\vmessage_ids\x18\x05 \x03(\tR\n" +
	"messageIds\x125\n" +
	"\bnext_run\x18\x06 \x01(\v2\x1a.google.protobuf.TimestampR\anextRun\x125\n" +
	"\blast_run\x18\a \x01(\v2\x1a.google.protobuf.TimestampR\alastRun\x129\n" +
	"\n" +
	"created_at\x18\b \x01(\v2\x1a.google.protobuf.TimestampR\tcreatedAt\x129\n" +
	"\n" +
	"updated_at\x18\t \x01(\v2\x1a.google.protobuf.TimestampR\tupdatedAt\x12'\n" +
	"\x0fexclusivity_key\x18\n" +
	" \x01(\tR\x0eexclusivityKey\x12#\n" +
	"\rstate_message\x18\v \x01(\tR\fstateMessage\x12\x1a\n" +
	"\bpriority\x18\f \x01(\x03R\bpriority\x12(\n" +
	"\x10has_max_messages\x18\r \x01(\bR\x0ehasMaxMessages\x12!\n" +
	"\fmax_messages\x18\x0e \x01(\x03R\vmaxMessages\x12@\n" +
	"\x0elease_duration\x18\x0f \x01(\v2\x19.google.protobuf.DurationR\rleaseDuration\x12\x1a\n" +
	"\btimezone\x18\x11 \x01(\tR\btimezone\x127\n" +
	"\tnext_runs\x18\x12 \x03(\v2\x1a.google.protobuf.TimestampR\bnextRuns\"=\n" +
	"\x05State\x12\r\n" +
	"\tSCHEDULED\x10\x00\x12\f\n" +
	"\bCANCELED\x10\x01\x12\v\n" +
	"\aERRORED\x10\x02\x12\n" +
	"\n" +
	"\x06PAUSED\x10\x03B\x11\n" +
	"\x0fschedule_config\"\xd7\x02\n" +
	"\x0fScheduleHistory\x12?\n" +
	"\bmessages\x18\x01 \x03(\v2#.chronoqueue.api.message.v1.MessageR\bmessages\x12\x1f\n" +
	"\vschedule_id\x18\x02 \x01(\tR\n" +
	"scheduleId\x125\n" +
	"\bnext_run\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\anextRun\x125\n" +
	"\blast_run\x18\x04 \x01(\v2\x1a.google.protobuf.TimestampR\alastRun\x129\n" +
	"\n" +
	"created_at\x18\x05 \x01(\v2\x1a.google.protobuf.TimestampR\tcreatedAt\x129\n" +
	"\n" +
	"updated_at\x18\x06 \x01(\v2\x1a.google.protobuf.TimestampR\tupdatedAt\"\xca\x03\n" +
	"\x10CalendarSchedule\x12N\n" +
	"\x04type\x18\x01 \x01(\x0e2:.chronoqueue.api.schedule.v1.CalendarSchedule.ScheduleTypeR\x04type\x12?\n" +
	"\x05rules\x18\x02 \x03(\v2).chronoqueue.api.schedule.v1.CalendarRuleR\x05rules\x12\x1a\n" +
	"\btimezone\x18\x03 \x01(\tR\btimezone\x12Z\n" +
	"\x11business_calendar\x18\x04 \x01(\v2-.chronoqueue.api.schedule.v1.BusinessCalendarR\x10businessCalendar\x12N\n" +
	"\n" +
	"exceptions\x18\x05 \x03(\v2..chronoqueue.api.schedule.v1.CalendarExceptionR\n" +
	"exceptions\"]\n" +
	"\fScheduleType\x12\v\n" +
	"\aMONTHLY\x10\x00\x12\n" +
	"\n" +
	"\x06WEEKLY\x10\x01\x12\t\n" +
	"\x05DAILY\x10\x02\x12\n" +
	"\n" +
	"\x06YEARLY\x10\x03\x12\x11\n" +
	"\rBUSINESS_DAYS\x10\x04\x12\n" +
	"\n" +
	"\x06CUSTOM\x10\x05\"\x84\x05\n" +
	"\fCalendarRule\x12D\n" +
	"\amonthly\x18\x01 \x01(\v2(.chronoqueue.api.schedule.v1.MonthlyRuleH\x00R\amonthly\x12A\n" +
	"\x06weekly\x18\x02 \x01(\v2'.chronoqueue.api.schedule.v1.WeeklyRuleH\x00R\x06weekly\x12>\n" +
	"\x05daily\x18\x03 \x01(\v2&.chronoqueue.api.schedule.v1.DailyRuleH\x00R\x05daily\x12A\n" +
	"\x06yearly\x18\x04 \x01(\v2'.chronoqueue.api.schedule.v1.YearlyRuleH\x00R\x06yearly\x12T\n" +
	"\rbusiness_days\x18\x05 \x01(\v2-.chronoqueue.api.schedule.v1.BusinessDaysRuleH\x00R\fbusinessDays\x12A\n" +
	"\x06custom\x18\x06 \x01(\v2'.chronoqueue.api.schedule.v1.CustomRuleH\x00R\x06custom\x12O\n" +
	"\x0fexecution_times\x18\a \x03(\v2&.chronoqueue.api.schedule.v1.TimeOfDayR\x0eexecutionTimes\x129\n" +
	"\n" +
	"valid_from\x18\b \x01(\v2\x1a.google.protobuf.TimestampR\tvalidFrom\x12;\n" +
	"\vvalid_until\x18\t \x01(\v2\x1a.google.protobuf.TimestampR\n" +
	"validUntilB\x06\n" +
	"\x04rule\"\x82\x02\n" +
	"\vMonthlyRule\x12K\n" +
	"\bday_type\x18\x01 \x01(\x0e20.chronoqueue.api.schedule.v1.MonthlyRule.DayTypeR\adayType\x12\x1b\n" +
	"\tday_value\x18\x02 \x01(\x05R\bdayValue\x12\x1e\n" +
	"\n" +
	"occurrence\x18\x03 \x01(\x05R\n" +
	"occurrence\x12\x16\n" +
	"\x06months\x18\x04 \x03(\x05R\x06months\"Q\n" +
	"\aDayType\x12\x10\n" +
	"\fDAY_OF_MONTH\x10\x00\x12\x14\n" +
	"\x10WEEKDAY_OF_MONTH\x10\x01\x12\x10\n" +
	"\fLAST_WEEKDAY\x10\x02\x12\f\n" +
	"\bLAST_DAY\x10\x03\"\x8e\x01\n" +
	"\n" +
	"WeeklyRule\x12 \n" +
	"\fdays_of_week\x18\x01 \x03(\x05R\n" +
	"daysOfWeek\x12#\n" +
	"\rweek_interval\x18\x02 \x01(\x05R\fweekInterval\x129\n" +
	"\n" +
	"start_week\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\tstartWeek\"\x8e\x01\n" +
	"\tDailyRule\x12!\n" +
	"\fday_interval\x18\x01 \x01(\x05R\vdayInterval\x12#\n" +
	"\rweekdays_only\x18\x02 \x01(\bR\fweekdaysOnly\x129\n" +
	"\n" +
	"start_date\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\tstartDate\"e\n" +
	"\n" +
	"YearlyRule\x12\x14\n" +
	"\x05month\x18\x01 \x01(\x05R\x05month\x12\x10\n" +
	"\x03day\x18\x02 \x01(\x05R\x03day\x12/\n" +
	"\x14adjust_for_leap_year\x18\x03 \x01(\bR\x11adjustForLeapYear\"c\n" +
	"\x10BusinessDaysRule\x120\n" +
	"\x14business_calendar_id\x18\x01 \x01(\tR\x12businessCalendarId\x12\x1d\n" +
	"\n" +
	"day_offset\x18\x02 \x01(\x05R\tdayOffset\"\xe1\x01\n" +
	"\n" +
	"CustomRule\x12\x1e\n" +
	"\n" +
	"expression\x18\x01 \x01(\tR\n" +
	"expression\x12\x1b\n" +
	"\trule_type\x18\x02 \x01(\tR\bruleType\x12W\n" +
	"\n" +
	"parameters\x18\x03 \x03(\v27.chronoqueue.api.schedule.v1.CustomRule.ParametersEntryR\n" +
	"parameters\x1a=\n" +
	"\x0fParametersEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"O\n" +
	"\tTimeOfDay\x12\x12\n" +
	"\x04hour\x18\x01 \x01(\x05R\x04hour\x12\x16\n" +
	"\x06minute\x18\x02 \x01(\x05R\x06minute\x12\x16\n" +
	"\x06second\x18\x03 \x01(\x05R\x06second\"\xea\x01\n" +
	"\x10BusinessCalendar\x12\x1f\n" +
	"\vcalendar_id\x18\x01 \x01(\tR\n" +
	"calendarId\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12 \n" +
	"\vdescription\x18\x03 \x01(\tR\vdescription\x12@\n" +
	"\bholidays\x18\x04 \x03(\v2$.chronoqueue.api.schedule.v1.HolidayR\bholidays\x12!\n" +
	"\fweekend_days\x18\x05 \x03(\x05R\vweekendDays\x12\x1a\n" +
	"\btimezone\x18\x06 \x01(\tR\btimezone\"\xb6\x01\n" +
	"\aHoliday\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12.\n" +
	"\x04date\x18\x02 \x01(\v2\x1a.google.protobuf.TimestampR\x04date\x12)\n" +
	"\x10recurring_yearly\x18\x03 \x01(\bR\x0frecurringYearly\x12<\n" +
	"\x04rule\x18\x04 \x01(\v2(.chronoqueue.api.schedule.v1.HolidayRuleR\x04rule\"\xf0\x01\n" +
	"\vHolidayRule\x12>\n" +
	"\x05fixed\x18\x01 \x01(\v2&.chronoqueue.api.schedule.v1.FixedDateH\x00R\x05fixed\x12G\n" +
	"\brelative\x18\x02 \x01(\v2).chronoqueue.api.schedule.v1.RelativeDateH\x00R\brelative\x12P\n" +
	"\reaster_offset\x18\x03 \x01(\v2).chronoqueue.api.schedule.v1.EasterOffsetH\x00R\feasterOffsetB\x06\n" +
	"\x04rule\"3\n" +
	"\tFixedDate\x12\x14\n" +
	"\x05month\x18\x01 \x01(\x05R\x05month\x12\x10\n" +
	"\x03day\x18\x02 \x01(\x05R\x03day\"^\n" +
	"\fRelativeDate\x12\x14\n" +
	"\x05month\x18\x01 \x01(\x05R\x05month\x12\x18\n" +
	"\aweekday\x18\x02 \x01(\x05R\aweekday\x12\x1e\n" +
	"\n" +
	"occurrence\x18\x03 \x01(\x05R\n" +
	"occurrence\"/\n" +
	"\fEasterOffset\x12\x1f\n" +
	"\vdays_offset\x18\x01 \x01(\x05R\n" +
	"daysOffset\"\xed\x02\n" +
	"\x11CalendarException\x12.\n" +
	"\x04date\x18\x01 \x01(\v2\x1a.google.protobuf.TimestampR\x04date\x12P\n" +
	"\x04type\x18\x02 \x01(\x0e2<.chronoqueue.api.schedule.v1.CalendarException.ExceptionTypeR\x04type\x12?\n" +
	"\rreschedule_to\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\frescheduleTo\x12G\n" +
	"\vextra_times\x18\x04 \x03(\v2&.chronoqueue.api.schedule.v1.TimeOfDayR\n" +
	"extraTimes\x12\x16\n" +
	"\x06reason\x18\x05 \x01(\tR\x06reason\"4\n" +
	"\rExceptionType\x12\b\n" +
	"\x04SKIP\x10\x00\x12\x0e\n" +
	"\n" +
	"RESCHEDULE\x10\x01\x12\t\n" +
	"\x05EXTRA\x10\x02B:Z8github.com/adrien19/chronoqueue/api/schedule/v1;scheduleb\x06proto3"

var (
	file_proto_schedule_v1_schedule_proto_rawDescOnce sync.Once
	file_proto_schedule_v1_schedule_proto_rawDescData []byte
)

func file_proto_schedule_v1_schedule_proto_rawDescGZIP() []byte {
	file_proto_schedule_v1_schedule_proto_rawDescOnce.Do(func() {
		file_proto_schedule_v1_schedule_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_proto_schedule_v1_schedule_proto_rawDesc), len(file_proto_schedule_v1_schedule_proto_rawDesc)))
	})
	return file_proto_schedule_v1_schedule_proto_rawDescData
}

var file_proto_schedule_v1_schedule_proto_enumTypes = make([]protoimpl.EnumInfo, 4)
var file_proto_schedule_v1_schedule_proto_msgTypes = make([]protoimpl.MessageInfo, 20)
var file_proto_schedule_v1_schedule_proto_goTypes = []any{
	(Schedule_Metadata_State)(0),         // 0: chronoqueue.api.schedule.v1.Schedule.Metadata.State
	(CalendarSchedule_ScheduleType)(0),   // 1: chronoqueue.api.schedule.v1.CalendarSchedule.ScheduleType
	(MonthlyRule_DayType)(0),             // 2: chronoqueue.api.schedule.v1.MonthlyRule.DayType
	(CalendarException_ExceptionType)(0), // 3: chronoqueue.api.schedule.v1.CalendarException.ExceptionType
	(*Schedule)(nil),                     // 4: chronoqueue.api.schedule.v1.Schedule
	(*ScheduleHistory)(nil),              // 5: chronoqueue.api.schedule.v1.ScheduleHistory
	(*CalendarSchedule)(nil),             // 6: chronoqueue.api.schedule.v1.CalendarSchedule
	(*CalendarRule)(nil),                 // 7: chronoqueue.api.schedule.v1.CalendarRule
	(*MonthlyRule)(nil),                  // 8: chronoqueue.api.schedule.v1.MonthlyRule
	(*WeeklyRule)(nil),                   // 9: chronoqueue.api.schedule.v1.WeeklyRule
	(*DailyRule)(nil),                    // 10: chronoqueue.api.schedule.v1.DailyRule
	(*YearlyRule)(nil),                   // 11: chronoqueue.api.schedule.v1.YearlyRule
	(*BusinessDaysRule)(nil),             // 12: chronoqueue.api.schedule.v1.BusinessDaysRule
	(*CustomRule)(nil),                   // 13: chronoqueue.api.schedule.v1.CustomRule
	(*TimeOfDay)(nil),                    // 14: chronoqueue.api.schedule.v1.TimeOfDay
	(*BusinessCalendar)(nil),             // 15: chronoqueue.api.schedule.v1.BusinessCalendar
	(*Holiday)(nil),                      // 16: chronoqueue.api.schedule.v1.Holiday
	(*HolidayRule)(nil),                  // 17: chronoqueue.api.schedule.v1.HolidayRule
	(*FixedDate)(nil),                    // 18: chronoqueue.api.schedule.v1.FixedDate
	(*RelativeDate)(nil),                 // 19: chronoqueue.api.schedule.v1.RelativeDate
	(*EasterOffset)(nil),                 // 20: chronoqueue.api.schedule.v1.EasterOffset
	(*CalendarException)(nil),            // 21: chronoqueue.api.schedule.v1.CalendarException
	(*Schedule_Metadata)(nil),            // 22: chronoqueue.api.schedule.v1.Schedule.Metadata
	nil,                                  // 23: chronoqueue.api.schedule.v1.CustomRule.ParametersEntry
	(*v1.Message)(nil),                   // 24: chronoqueue.api.message.v1.Message
	(*timestamppb.Timestamp)(nil),        // 25: google.protobuf.Timestamp
	(*v11.Payload)(nil),                  // 26: chronoqueue.api.common.v1.Payload
	(*durationpb.Duration)(nil),          // 27: google.protobuf.Duration
}
var file_proto_schedule_v1_schedule_proto_depIdxs = []int32{
	22, // 0: chronoqueue.api.schedule.v1.Schedule.metadata:type_name -> chronoqueue.api.schedule.v1.Schedule.Metadata
	24, // 1: chronoqueue.api.schedule.v1.ScheduleHistory.messages:type_name -> chronoqueue.api.message.v1.Message
	25, // 2: chronoqueue.api.schedule.v1.ScheduleHistory.next_run:type_name -> google.protobuf.Timestamp
	25, // 3: chronoqueue.api.schedule.v1.ScheduleHistory.last_run:type_name -> google.protobuf.Timestamp
	25, // 4: chronoqueue.api.schedule.v1.ScheduleHistory.created_at:type_name -> google.protobuf.Timestamp
	25, // 5: chronoqueue.api.schedule.v1.ScheduleHistory.updated_at:type_name -> google.protobuf.Timestamp
	1,  // 6: chronoqueue.api.schedule.v1.CalendarSchedule.type:type_name -> chronoqueue.api.schedule.v1.CalendarSchedule.ScheduleType
	7,  // 7: chronoqueue.api.schedule.v1.CalendarSchedule.rules:type_name -> chronoqueue.api.schedule.v1.CalendarRule
	15, // 8: chronoqueue.api.schedule.v1.CalendarSchedule.business_calendar:type_name -> chronoqueue.api.schedule.v1.BusinessCalendar
	21, // 9: chronoqueue.api.schedule.v1.CalendarSchedule.exceptions:type_name -> chronoqueue.api.schedule.v1.CalendarException
	8,  // 10: chronoqueue.api.schedule.v1.CalendarRule.monthly:type_name -> chronoqueue.api.schedule.v1.MonthlyRule
	9,  // 11: chronoqueue.api.schedule.v1.CalendarRule.weekly:type_name -> chronoqueue.api.schedule.v1.WeeklyRule
	10, // 12: chronoqueue.api.schedule.v1.CalendarRule.daily:type_name -> chronoqueue.api.schedule.v1.DailyRule
	11, // 13: chronoqueue.api.schedule.v1.CalendarRule.yearly:type_name -> chronoqueue.api.schedule.v1.YearlyRule
	12, // 14: chronoqueue.api.schedule.v1.CalendarRule.business_days:type_name -> chronoqueue.api.schedule.v1.BusinessDaysRule
	13, // 15: chronoqueue.api.schedule.v1.CalendarRule.custom:type_name -> chronoqueue.api.schedule.v1.CustomRule
	14, // 16: chronoqueue.api.schedule.v1.CalendarRule.execution_times:type_name -> chronoqueue.api.schedule.v1.TimeOfDay
	25, // 17: chronoqueue.api.schedule.v1.CalendarRule.valid_from:type_name -> google.protobuf.Timestamp
	25, // 18: chronoqueue.api.schedule.v1.CalendarRule.valid_until:type_name -> google.protobuf.Timestamp
	2,  // 19: chronoqueue.api.schedule.v1.MonthlyRule.day_type:type_name -> chronoqueue.api.schedule.v1.MonthlyRule.DayType
	25, // 20: chronoqueue.api.schedule.v1.WeeklyRule.start_week:type_name -> google.protobuf.Timestamp
	25, // 21: chronoqueue.api.schedule.v1.DailyRule.start_date:type_name -> google.protobuf.Timestamp
	23, // 22: chronoqueue.api.schedule.v1.CustomRule.parameters:type_name -> chronoqueue.api.schedule.v1.CustomRule.ParametersEntry
	16, // 23: chronoqueue.api.schedule.v1.BusinessCalendar.holidays:type_name -> chronoqueue.api.schedule.v1.Holiday
	25, // 24: chronoqueue.api.schedule.v1.Holiday.date:type_name -> google.protobuf.Timestamp
	17, // 25: chronoqueue.api.schedule.v1.Holiday.rule:type_name -> chronoqueue.api.schedule.v1.HolidayRule
	18, // 26: chronoqueue.api.schedule.v1.HolidayRule.fixed:type_name -> chronoqueue.api.schedule.v1.FixedDate
	19, // 27: chronoqueue.api.schedule.v1.HolidayRule.relative:type_name -> chronoqueue.api.schedule.v1.RelativeDate
	20, // 28: chronoqueue.api.schedule.v1.HolidayRule.easter_offset:type_name -> chronoqueue.api.schedule.v1.EasterOffset
	25, // 29: chronoqueue.api.schedule.v1.CalendarException.date:type_name -> google.protobuf.Timestamp
	3,  // 30: chronoqueue.api.schedule.v1.CalendarException.type:type_name -> chronoqueue.api.schedule.v1.CalendarException.ExceptionType
	25, // 31: chronoqueue.api.schedule.v1.CalendarException.reschedule_to:type_name -> google.protobuf.Timestamp
	14, // 32: chronoqueue.api.schedule.v1.CalendarException.extra_times:type_name -> chronoqueue.api.schedule.v1.TimeOfDay
	26, // 33: chronoqueue.api.schedule.v1.Schedule.Metadata.payload:type_name -> chronoqueue.api.common.v1.Payload
	0,  // 34: chronoqueue.api.schedule.v1.Schedule.Metadata.state:type_name -> chronoqueue.api.schedule.v1.Schedule.Metadata.State
	6,  // 35: chronoqueue.api.schedule.v1.Schedule.Metadata.calendar_schedule:type_name -> chronoqueue.api.schedule.v1.CalendarSchedule
	25, // 36: chronoqueue.api.schedule.v1.Schedule.Metadata.next_run:type_name -> google.protobuf.Timestamp
	25, // 37: chronoqueue.api.schedule.v1.Schedule.Metadata.last_run:type_name -> google.protobuf.Timestamp
	25, // 38: chronoqueue.api.schedule.v1.Schedule.Metadata.created_at:type_name -> google.protobuf.Timestamp
	25, // 39: chronoqueue.api.schedule.v1.Schedule.Metadata.updated_at:type_name -> google.protobuf.Timestamp
	27, // 40: chronoqueue.api.schedule.v1.Schedule.Metadata.lease_duration:type_name -> google.protobuf.Duration
	25, // 41: chronoqueue.api.schedule.v1.Schedule.Metadata.next_runs:type_name -> google.protobuf.Timestamp
	42, // [42:42] is the sub-list for method output_type
	42, // [42:42] is the sub-list for method input_type
	42, // [42:42] is the sub-list for extension type_name
	42, // [42:42] is the sub-list for extension extendee
	0,  // [0:42] is the sub-list for field type_name
}

func init() { file_proto_schedule_v1_schedule_proto_init() }
func file_proto_schedule_v1_schedule_proto_init() {
	if File_proto_schedule_v1_schedule_proto != nil {
		return
	}
	file_proto_schedule_v1_schedule_proto_msgTypes[3].OneofWrappers = []any{
		(*CalendarRule_Monthly)(nil),
		(*CalendarRule_Weekly)(nil),
		(*CalendarRule_Daily)(nil),
		(*CalendarRule_Yearly)(nil),
		(*CalendarRule_BusinessDays)(nil),
		(*CalendarRule_Custom)(nil),
	}
	file_proto_schedule_v1_schedule_proto_msgTypes[13].OneofWrappers = []any{
		(*HolidayRule_Fixed)(nil),
		(*HolidayRule_Relative)(nil),
		(*HolidayRule_EasterOffset)(nil),
	}
	file_proto_schedule_v1_schedule_proto_msgTypes[18].OneofWrappers = []any{
		(*Schedule_Metadata_CronSchedule)(nil),
		(*Schedule_Metadata_CalendarSchedule)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_proto_schedule_v1_schedule_proto_rawDesc), len(file_proto_schedule_v1_schedule_proto_rawDesc)),
			NumEnums:      4,
			NumMessages:   20,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_proto_schedule_v1_schedule_proto_goTypes,
		DependencyIndexes: file_proto_schedule_v1_schedule_proto_depIdxs,
		EnumInfos:         file_proto_schedule_v1_schedule_proto_enumTypes,
		MessageInfos:      file_proto_schedule_v1_schedule_proto_msgTypes,
	}.Build()
	File_proto_schedule_v1_schedule_proto = out.File
	file_proto_schedule_v1_schedule_proto_goTypes = nil
	file_proto_schedule_v1_schedule_proto_depIdxs = nil
}
