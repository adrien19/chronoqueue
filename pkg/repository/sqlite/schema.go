...
func (m *SchemaManager) createQueuesTable(ctx context.Context, tx *sql.Tx) error {
	_, err := tx.ExecContext(ctx, `CREATE TABLE IF NOT EXISTS cq_queues (name TEXT PRIMARY KEY, metadata_pb BLOB NOT NULL, state_counts TEXT DEFAULT '{}', created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL)`)
	return err
}
